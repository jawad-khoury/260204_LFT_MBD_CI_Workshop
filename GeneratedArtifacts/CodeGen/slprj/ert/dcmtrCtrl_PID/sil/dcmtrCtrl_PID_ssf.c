 ...
 * Component: dcmtrCtrl_PID
 * Component Simulink Path: dcmtrCtrl_PID
 * Simulation Mode: SIL
 *
 */

#define S_FUNCTION_NAME                dcmtrCtrl_PID_ssf
...
static RegMdlInfo rtMdlInfo_dcmtrCtrl_PID[1] = { "",
    ...
    if ((pIsXILApplicationStarted!=NULL) && (*pIsXILApplicationStarted==1)) {
...

      {
        ...
        if (IOBufferPtr != NULL) {
          void * pXILService = (void *) ssGetPWorkValue(S, 9);
          if (pXILService != NULL) {
            if (xilHostAppSvcGrowIOBuffer(pXILService, IOBufferPtr, 21, 0)!=
                XILHOSTAPPSVC_SUCCESS) {
              return;
            }                          /* if */
...
static boolean_T startAndSetupApplication(SimStruct *S)
{
...

  {
    ...
    {
      mxArray * codeGenComponent = mxCreateString("dcmtrCtrl_PID");
      mxArray *rhs[3];
      char * simulinkBlockPath = getSimulinkBlockPath(S);
      if (simulinkBlockPath == NULL) {
        ssSetErrorStatus(S,
...
static void mdlInitializeSizes(SimStruct *S)
{
  ...
  if (S->mdlInfo->genericFcn != (NULL) &&
    ...
    ssRegModelRefMdlInfo(S, (char *)"dcmtrCtrl_PID", rtMdlInfo_dcmtrCtrl_PID, 1);
  }
