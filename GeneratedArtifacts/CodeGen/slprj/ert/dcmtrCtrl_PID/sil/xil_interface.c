 ...
 *
 * SIL generated interface for code: "dcmtrCtrl_PID"
 *
 */
...
/* interface data */
dcmtrCtrl_PID_MdlrefDW dcmtrCtrl_PIDrtMdlrefDW;
static XILIOData xil_fcnid0_task1_output_u[3];
static XILIOData xil_fcnid0_task1_y[2];
static XILIOData xil_fcnid0_ws_params[5];
...
XIL_INTERFACE_ERROR_CODE xilOutput(uint32_T xilFcnId, uint32_T xilTID)
{
  ...
  switch (xilTID) {
   case 1:
    ...
    taskTimeStart_dcmtrCtrl_PID(1U);
    dcmtrCtrl_PID(&(i_ref), &(i_ang), &(o_v), &(dcmtrCtrl_PIDrtMdlrefDW.rtdw));
    taskTimeEnd_dcmtrCtrl_PID(1U);
